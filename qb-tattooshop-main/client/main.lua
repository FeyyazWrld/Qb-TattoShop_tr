...
function RemoveTattoo(name, label)
    ...
    print("~y~ Bu dövmeni sildirdin!" .. GetLabelText(label) .. "~s~ tattoo")
end

function CreateScale(sType)
    if scaleString ~= sType and sType == "OpenShop" then
        scaleType = setupScaleform("instructional_buttons", "Dövmeci dükkanını aç", 38)
        scaleString = sType
...
CreateThread(function()
    JayMenu.CreateMenu("tattoo", "Dövmeci", function()
        return CloseTattooShop()
    end)
    JayMenu.SetSubTitle('tattoo', "Katagoriler")
...
CreateThread(function()
    while true do
        ...
        for i = 1, #Config.Shops, 1 do
            ...
            if dist <= 8 then
                DrawText3Ds(coordinates.x, coordinates.y, coordinates.z + 0.10, "/tumdovmelerisil Bütün dövmelerini siler!")
            end
        end
    end
end)
...
RegisterCommand('tumdovmelerisil', function(source, args, rawCommand)
    for i = 1, #Config.Shops, 1 do
        local player = PlayerPedId()
        local playerloc = GetEntityCoords(player, 0)
        local coordinates = Config.Shops[i]
